...
def inference_on_generated_fol(generated_premises, generated_conclusion, gold_answer):
    ...
    answer, flag, error_message, _ = safe_execute_program(generated_premises, generated_conclusion)
...
class TranslatorEngine:
    def __init__(self, dataset, split, output_filename, batch_size, sample_limit=None, continuous=False):
        ...
        # Load model and tokenizer using Hugging Face AutoModel and AutoTokenizer
        ...
        # Instantiate FOLRefiner for continuous inference (kept as it is)
...
{}"""
    ...
    def translate_batch(self, batch_data, folio):
        ...
        for idx, prompt in enumerate(batch_prompt_inputs):
            ...
            # Split the base_prompt to identify where to insert the refinement string
            ...
            for i in range(3):
                ...
                prover_answer, prover_status, prover_error, is_correct, corrected_fol = None, None, None, None, None


                # Only run inference if continuous flag is True
                if self.continuous:
                    # First, run the logic prover for inference
                    ...
                    torch.cuda.empty_cache()
                    # Then, use the refiner to correct the generated FOL
                    ...
                    # Clear GPU memory after FOL correction
                    torch.cuda.empty_cache()

                    # Add iterative refinement string to the prompt for the next iteration
                    refinement_string = f"results for inference (round {i + 1}): {prover_answer}, {prover_status}, {prover_error}, {is_correct}\nFOL correction: {corrected_fol}\n\n"
            ...
            output_dict = {
                ...
                'corrected_fol': corrected_fol  # Store the corrected FOL after using the refiner
            }
...
if __name__ == '__main__':
    dataset = 'folio'
    split = 'train'
    output = 'prova_final_cont_folio_out.json'
    batch_size = 1
